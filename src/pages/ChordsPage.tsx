...
import { ChordsList, AutoSkipper, ChordsSettings } from '../components';

import { getListOfRandomChords, getRandomNoteFromCaged, isValidChordCountList } from '../services';
import { DEFAULT_NUMBER_OF_CHORD, CHORDS_LIST_MAX, CHORDS_LIST_MIN } from '../config/constants';
import { CagedType } from '../config';
import { MdBuild } from 'react-icons/md';

export const ChordsPage = () => {
  const [chords, setChords] = useState(getListOfRandomChords(DEFAULT_NUMBER_OF_CHORD));
  ...
  const ShapeDecorator: React.ReactNode = <div className={`${isShapeVisible ? '' : 'invisible'}`}>{cagedPosition}</div>;

  return (
    <div className="h-full flex flex-col items-center justify-center">
      <div className="w-screen flex items-center justify-center">
        <div className="flex flex-col items-center">
          <ChordsList chords={chords} ShapeDecorator={ShapeDecorator} onClick={handleGetRandomChordsOnClick} />
          <ChordsSettings />
          <div className="flex flex-col gap-1">
            <ButtonGroup isAttached>
              ...
              />
              <Button onClick={handleGetRandomChordsOnClick}>Generate list of {numberOfChordDisplayed} chords</Button>
              <IconButton
                aria-label="plus"
                icon={<FaPlus />}
                onClick={() => handleChangeNumberOfChordDisplayed(1)}
            ...
            <Button leftIcon={<MdBuild />} size="xs" onClick={toggleCagedVisible}>
